    ...
    while (!Q.empty()) {
        ...
        reps(i, -1, 2) {
            reps(j, -1, 2) {
                ...
                ll nx = U.a + i, ny = U.b + j;
                if (nx < 0 || nx >= N || ny < 0 || ny >= M) continue;
                if (((U.c == 0) && (A[nx][ny] == 'x')) ||
                    ((U.c == 1) && (A[nx][ny] == 'o')) || A[nx][ny] == '#')
                    continue;
                ll nc = U.c ^ (A[nx][ny] == '?');
                if (dp[nx][ny][nc] > dp[U.a][U.b][U.c] + 1) {
                    dp[nx][ny][nc] = dp[U.a][U.b][U.c] + 1;
                    Q.emplace(nx, ny, nc);
                }
            }
        }
    }
  if(*min_element(ALL(dp[fx][fy]))!=1e18)   cout << *min_element(ALL(dp[fx][fy])) << '\n';
  else cout << -1 <<'\n';
}
int main() {
    ll a = 0, b = 0;
